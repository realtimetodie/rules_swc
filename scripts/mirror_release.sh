...
pkgs=$(curl -sSL -H 'Accept: application/vnd.github.v3+json' https://api.github.com/repos/swc-project/swc/releases?per_page=10 | jq -f $(pwd)/scripts/filter.jq)
...
for version in $versions; do
    ...
    for asset in $assets; do
        ...
        echo "        \"$asset\": \"$(curl -sSL $url | shasum -b -a 384 | awk "{ print \$1 }" | xxd -r -p | base64)\","
    done
    echo "    },"
done
echo "}"
